...
## 6. 最近改动记录
- **2026-09-24**：优化时间轴“往年今日”纯文字日记卡片对比度：
  - 实施自适应卡片主题：有图时保留唯美照片暗色蒙层与白字；无图时自动切换为温馨信纸暖白渐变卡片（`.no-img`）；
  - 文字自适应转为小窝主题暖深褐色（`#5A4636` / `#5D4A3E`）与粉色时间标签（`#F48FB1`），彻底解决纯文字日记白字对比度过低、看不清的问题。
