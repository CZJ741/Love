...
## 6. 最近改动记录
- **2026-09-24**：情侣定位共享功能精炼文案并强化最新位置更新时间：
  - 遵循极简精炼原则，彻底移除“已获取经纬度定位”、“已共享最新位置”及顶部多余说明赘述，将最新更新时间直接提至地址副文本位置展示（如 `更新于 刚刚` / `更新于 3分钟前`）；
  - 地图标注点气泡同步附带紧凑时间标签，界面更加干净自解释。
...
- **2026-09-24**：实现房间成立时（伴侣加入小窝后）房主首次设定恋爱纪念日与单向锁定机制：
  ...
  - 小程序前端：在 `miniprogram/pages/home` 增加双方到齐时房主主动唤起的极简选日弹窗与未锁定时点击卡片设定入口，设定完成后卡片展示真实相爱天数，永久关闭修改入口。
